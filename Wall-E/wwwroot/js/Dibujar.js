...
window.obtenerDimensionesCanvas = function (canvasRef) {
    const canvas = canvasRef.firstElementChild;

    // Verificar si el canvas está completamente cargado
    if (canvas.complete) {
        // Si está completamente cargado, obtener las dimensiones
        const width = canvas.width;
        const height = canvas.height;
        return { width, height };
    } else {
        // Si no está completamente cargado, esperar al evento load
...
window.crearPuntoEnCanvas = function(x,y) {
        ...
        var punto = new paper.Path.Circle(new paper.Point(x, y), 1);
